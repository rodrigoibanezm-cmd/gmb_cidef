...
export async function buildLocationIndexes(date) {
  ...
  for (const [location, places] of Object.entries(byLocation)) {
    ...
    const summary = {
      ...
      owned_count: ranking.filter((item) => item.ownership_group === "cidef" || item.ownership_group === "own").length,
  ...
  return {
    ...
    ranked_locations: locations.filter((item) => item.stores_count > 0).length,
  };
}
