...
function stripReviews(result) {
  return {
    ...result,
    ranking: result.ranking.map(({ reviews, ...item }) => item),
  };
}
...
export async function compareMallFromSnapshot({ mall, date = today(), includeReviews = false, evidenceLimit = 3 }) {
  ...
  for (const place of stores) {
    ...
    const rawReviews = await getReviews(date, place.place_id);
    const reviews = cleanReviews(rawReviews, evidenceLimit);
    ...
    if (includeReviews) item.reviews = reviews;
    results.push(item);
  }
...
export async function compareAllFromSnapshot({ date = today() } = {}) {
  ...
  for (const mall of Object.keys(placesByLocation)) {
    const comparison = await compareMallFromSnapshot({ mall, date, includeReviews: false });
    malls.push(stripReviews(comparison));
  }
