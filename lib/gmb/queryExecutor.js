...
function applyFilters(rows, filters) {
  return rows.filter((row) => {
    ...
    if (filters.ownership_group !== "all" && row.ownership_group !== filters.ownership_group) return false;
...
async function executeExtra(query) {
  const rows = applyFilters(await readLocationSummaries(query.date), query.filters);
...
async function executeIntra(query) {
  ...
  for (const location of locations) {
    ...
    allRows.push(...applyFilters(ranking, query.filters));
  }
