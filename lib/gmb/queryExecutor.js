...
function isOwn(value) {
  return value === "own" || value === "cidef";
}

function matchesOwnership(rowValue, filterValue) {
  if (filterValue === "all") return true;
  if (filterValue === "own") return isOwn(rowValue);
...
function applyEntityFilters(rows, filters) {
  return rows.filter((row) => {
    ...
    if (!matchesOwnership(row.ownership_group, filters.ownership_group)) return false;
