...
export async function getReviewEvidence({ date, placeId, limit = 3, tenantId = "cidef" }) {
  if (!date || !placeId) return [];

  const rawKeys = await redisCommand(["GET", gmbCaptureKeys.placeReviewIndex(date, placeId, tenantId)]);
