...
async function executeTemporalAggregate(query, internalOutput) {
  const base = { ...query, metric: query.metric.replace(/^delta_/, "") };
  ...
  for (const to of toRows) {
    ...
    rows.push({
      ...
      delta_rating: query.metric === "delta_rating" ? roundDelta(Number(to.avg_value) - Number(from.avg_value)) : null,
      delta_reviews_count: query.metric === "delta_reviews_count" ? roundDelta(Number(to.avg_value) - Number(from.avg_value)) : null,
      delta_gap_vs_top: query.metric === "delta_gap_vs_top" ? roundDelta(Number(to.avg_value) - Number(from.avg_value)) : null,
      delta_position: query.metric === "delta_position" ? roundDelta(Number(to.avg_value) - Number(from.avg_value)) : null,
