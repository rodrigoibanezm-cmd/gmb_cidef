import { attachEvidence } from "./evidenceAttach.js";
...
function hasExactPlaceFilter(filters = {}) {
  return Boolean(filters.place_id || filters.place_name || filters.mall_name || filters.review_hash);
}

function buildExactPlaceRows(metricRows) {
  return metricRows.map((row) => {
    const enriched = withConfidence(row);
    return {
      place_id: enriched.place_id,
      name: enriched.name,
      brand: enriched.brand,
      location: enriched.normalized_location || enriched.location,
      normalized_location: enriched.normalized_location,
      market_group: enriched.market_group,
      region: enriched.region,
      operator: enriched.operator,
      store_role: enriched.store_role,
      ownership_group: enriched.ownership_group,
      captured_date: enriched.captured_date,
      rating: enriched.rating,
      reviews_count: enriched.reviews_count,
      reviews: enriched.reviews_count,
...
async function executeExtra(query) {
  ...
  if (hasExactPlaceFilter(query.filters)) {
    return buildExactPlaceRows(metricRows).slice(0, query.output.max_rows);
  }
