...
function indexByLocation(rows) {
  const map = new Map();
  for (const row of rows) {
    if (row?.location) map.set(row.location, row);
  }
  return map;
}
...
async function executeTemporal(query) {
  const fromRows = await executeExtra({
    ...
    output: { ...query.output, include_evidence: false, max_rows: 200 },
  });
  ...
  const fromByLocation = indexByLocation(fromRows);
  const rows = [];
